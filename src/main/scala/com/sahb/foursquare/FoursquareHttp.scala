...
object FoursquareHttp extends LQLogger {
	...
	def redirectUri:String = {
      Props.get("foursquare.callback") match {
         case Full(url) => Helpers.urlEncode("http://" + url + "/foursquare/callback.html")
         case _ => Helpers.urlEncode("http://localhost:8080/foursquare/callback.html")
      }
   }
